...
- **资产**：股票、基金/ETF、现金、黄金、外汇、债券、商品、REIT、可转债、期货、期权、互换及结构化产品；每类连接权利义务、现金流、官方来源和可复现实验。
- **行业**：通用分类与经营桥 → 存储、能源、金融/保险、消费、医药、互联网、硬件半导体、工业资本品、地产及材料矿业。
- **公司**：跨市场法定披露与历史时点 → 三表勾稽、收入和现金转换 → 治理、资本配置、稀释 → 反向 DCF、情景与可比估值。
- **方法**：预注册与证据等级 → 时间序列/走步与事件研究 → 因果识别 → 因子、回测、数据质量与负结果。
- **组合**：目标与投资政策 → 配置、分散和再平衡 → 流动性、风险贡献和压力测试 → 基准、归因与过程日志。
...
- `src/investkb/market_operations.py`：订单、费用、盘口、公司行动和结算日的有效期参数化练习。
- `src/investkb/market_calendar.py`：时区会话、日历漂移检测和无凭据全球市场 smoke。
