...
- [x] Test a known pooled-standard-deviation effect size, monotonic power, invalid inputs, full-rank OLS diagnostics, leverage, HC1 errors, influence, and singular rejection.
- [x] Run `.venv/bin/pytest -q tests/test_advanced_foundations.py` and confirm import failure.
- [x] Commit the red contract before production implementation.
- [x] Implement the smallest validated numerical functions and rerun focused tests and Ruff.
...
- [x] Test equal results for equal seeds, different results for different seeds, output size, non-finite data, invalid resample counts, and missing/bool seed rejection.
- [x] Verify the test fails before implementation.
- [x] Implement a local `numpy.random.Generator` bootstrap without touching global random state.
- [x] Connect the exercise to the checked dependency and interpreter environment.
...
- [x] Verify current primary/open textbook URLs and record retrieval, license, scope, and local-use boundaries.
- [x] Add worked examples, falsification questions, and misuse warnings rather than source-only link lists.
- [x] Link every new page from the Wiki index and MkDocs navigation.
...
- [x] Reuse the frozen public evidence-case implementation/test for paper reproduction and the tested company reconciliation for accounting.
- [x] Promote all twelve incomplete foundation requirements only after stage-appropriate evidence exists.
- [x] Assert all twenty foundation requirements are validated and have no gap.
- [x] Regenerate reports and public indexes.

### Task 5: Verification and delivery

- [x] Run `./scripts/verify.sh` and require all tests, source checks, Wiki lint, coverage, privacy audit, strict MkDocs, and offline demo to pass.
